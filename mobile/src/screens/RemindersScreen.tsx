...
export default function RemindersScreen() {
  ...
  const getRemindersSpeech = useCallback((): string => {
    const pending = reminders.filter((r) => !r.done);
    if (pending.length === 0) return t('tts.noReminders');
    const prefix = t('tts.reminderPrefix');
    return pending.map((r) => `${prefix} ${r.title} at ${r.time}`).join('. ');
  }, [reminders, t]);

  const handleVoiceMarkDone = useCallback((): boolean => {
    const target = reminders.find((r) => !r.done);
    if (!target) return false;
    handleMarkDone(target.id);
    return true;
  }, [reminders]);
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <VoiceMicButton
        getRemindersSpeech={getRemindersSpeech}
        onMarkDone={handleVoiceMarkDone}
      />
